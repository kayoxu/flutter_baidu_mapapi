...
class BMFMapStatusDispatcher {
  ...
  Future<bool?> setScrollBy(
      MethodChannel? _mapChannel, int? xPixel, int? yPixel,
  ...
  Future<Uint8List?> takeSnapshot(MethodChannel? _mapChannel) async {
    ...
    var list = (result?.map((i) => i as int).toList()) ?? [];
  ...
  Future<bool?> setCustomMapStyleWithOption(
    ...
    try {
      Map? map = (await _mapChannel.invokeMethod(
          ...
          {'customMapStyleOption': customMapStyleOption.toMap()} as dynamic));
      result = map != null;
