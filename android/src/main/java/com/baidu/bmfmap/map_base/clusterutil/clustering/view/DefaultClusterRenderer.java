...
package com.baidu.bmfmap.map_base.clusterutil.clustering.view;

import static com.baidu.bmfmap.map_base.clusterutil.clustering.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;
...
import com.baidu.bmfmap.map.overlayHandler.MarkerHandler;
...
import com.baidu.bmfmap.map_base.clusterutil.MarkerManager;
...
import com.baidu.bmfmap.map_base.clusterutil.projection.Point;
import com.baidu.bmfmap.map_base.clusterutil.projection.SphericalMercatorProjection;
import com.baidu.bmfmap.map_base.clusterutil.ui.IconGenerator;
import com.baidu.bmfmap.map_base.clusterutil.ui.SquareTextView;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements
    ...
    public DefaultClusterRenderer(Context context, BaiduMap map, ClusterManager<T> clusterManager) {
        mViewModifier.setContext(context);
        mMap = map;
...
//        outline.getPaint().setColor(0x80ffffff); // Transparent white.
    ...
    private class ViewModifier extends Handler {
        ...
        public void queue(Set<? extends Cluster<T>> clusters) {
            synchronized (this) {
                ...
                mNextClusters.setContext(context);
            }
            sendEmptyMessage(RUN_TASK);
        }
    }
    ...
    private class RenderTask implements Runnable {
        ...
        public void run() {
            ...
            markerModifier.setContext(context);
    ...
    private class MarkerModifier extends Handler implements MessageQueue.IdleHandler {
        ...
        private void performNextTask() {
            if (!mOnScreenRemoveMarkerTasks.isEmpty()) {
                ...
                mOnScreenCreateMarkerTasks.poll().perform(context, this);
            } else if (!mCreateMarkerTasks.isEmpty()) {
                mCreateMarkerTasks.poll().perform(context, this);
     ...
     */
    protected void onBeforeClusterRendered(Cluster<T> cluster, MarkerOptions markerOptions, int zIndex) {
    ...
    private class CreateMarkerTask {
        ...
        private void perform(Context context, MarkerModifier markerModifier) {
...
//                            markerOptions.icon(item.getBitmapDescriptor());
                            if (item instanceof VMCarItem) {
//                                markerOptions.rotate(((VMCarItem) item).getRotate()).anchor(0.5f, 0.5f).zIndex(((VMCarItem) item).getzIndex());
                                 if (extracted(context, (VMCarItem) item, markerOptions))  {
//                                    return;
                                };
                             }
                            //添加额外信息
...
//                            markerOptions.icon(item.getBitmapDescriptor());
                            ...
                            if (item instanceof VMCarItem) {
//                                markerOptions.rotate(((VMCarItem) item).getRotate()).anchor(0.5f, 0.5f).zIndex(((VMCarItem) item).getzIndex());
//                                markerOptions = ((VMCarItem) item).getMarkerOptions();
                                if (extracted(context, (VMCarItem) item, markerOptions)) {
//                                    return;
                                }
                            }
            ...
            if (null != cluster.getItems() && cluster.getItems().size() > 0) {
                for (T item : cluster.getItems()) {
                    if (item instanceof VMCarItem) {
                        zIndex = ((VMCarItem) item).getzIndex() + 1;
//                        zIndex = ((VMCarItem) item).getzIndex();
                    }
                    break;
                }
            }
            ...
            onBeforeClusterRendered(cluster, markerOptions, zIndex);
        ...
        private boolean extracted(Context context, VMCarItem item, MarkerOptions markerOptions) {
            Map<String, Object> argument = item.getArgument();
            if (null == argument) {
                return true;
            }
            ...
            if (!MarkerHandler.setMarkerOptions2(context, argument, markerOptions, id, icon)) {
                return true;
            }
            MarkerHandler.setScreenLockPoint(argument, markerOptions);
            return false;
        }
    }
